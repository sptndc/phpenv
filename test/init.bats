...
@test "setup shell completions (fish)" {
  ...
  assert_success
  assert_line ". '${root}/test/../libexec/../completions/phpenv.fish'"
}

@test "fish instructions" {
  run phpenv-init fish
  assert [ "$status" -eq 1 ]
  assert_line 'status --is-interactive; and . (phpenv init -|psub)'
}
...
@test "adds shims to PATH (fish)" {
  ...
  run phpenv-init - fish
  assert_success
  assert_line 0 "setenv PATH '${PHPENV_ROOT}/shims' \$PATH"
}
